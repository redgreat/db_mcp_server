...
**返回解读：**
- markdown 格式：返回 `content` 字段，包含完整的 Markdown 文本，可直接展示
- pdf 格式：返回 base64 编码的 PDF 数据

**最佳实践：**
- 默认用 markdown，用户可直接看
- 用户如果说"想下载"或"发给别人"时用 pdf
...
```
1. list_connections → 确认连接
2. export_db_doc(format="markdown") → 生成文档
3. 展示 Markdown 文档内容
4. 如果用户需要 PDF → export_db_doc(format="pdf")
```
