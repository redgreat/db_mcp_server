...
本文件为 AI IDE（Cursor、Trae 等）提供 MCP 工具调用指南。工具定义以 `src/mcp/tools.py` 为准；人类可读说明见 `README.md` 与 `docs/MCP客户端调用说明.md`。
...
**支持的数据库类型**：`mysql`、`postgresql`（`list_connections` 的 `search` 勿用 `mssql` 等未支持类型筛选）。
...
| `execute_sql` | 执行任意 SQL | `connection_id`, `sql` | 写/DDL | |
| `export_db_doc` | 导出数据字典 | `connection_id` | 读 | |
| `generate_er_diagram` | 生成 ER 图 | `connection_id` | 读 | 可选 |
| `generate_data_flow` | 生成数据流图 | `connection_id` | 读 | |
| `suggest_columns` | 字段添加建议 | `connection_id`, `table` | 读 | 可选 |
| `analyze_performance` | 性能分析 | `connection_id` | 读 | 可选 |
| `analyze_sql` | SQL 审查 | `connection_id`, `sql` | 读 | 可选 |
| `analyze_db_config` | 参数调优分析 | `connection_id` | 读 | 可选 |
...
| `generate_db_rule` | 生成 RULE.md | `connection_id` | 读 | |
| `render_report_oss` | 报表上传 OSS | `connection_id`, `source`, `fields` | 读 | |

**LLM 说明**：需在服务端 Web 管理端 **「大模型配置」** 激活模型。启用后 `analyze_sql` 返回 `ai_assessment` + `ai_usage`；`analyze_performance` / `analyze_db_config` 返回 `ai_analysis`；`suggest_columns`（带 `columns`）返回 `ai_suggestions`；`generate_er_diagram` 含业务域文字解读。未启用时仍有规则/EXPLAIN 结果，AI 字段为禁用提示。

**OSS 说明**：`export_db_doc`、`generate_er_diagram`、`generate_data_flow`、`render_report_oss` 支持 `upload_to_oss`；服务端 `object_storage.enabled=true` 时默认上传并返回 `download_url` 等字段，应优先把链接交给用户而非粘贴全文。
...
- `save_path`（可选）：服务端绝对路径，直接落盘
- `upload_to_oss`（可选）：是否上传 OSS；默认随 `object_storage.enabled`
...
// 上传 OSS 并返回下载链接
{"connection_id": 1, "database": "mydb", "format": "pdf", "upload_to_oss": true}
```
...
**分片输出与本地保存（重要）：**
- 当文档较大导致 IDE 展示截断或不便阅读时，采用“分片保存到本地”的策略。
- 保存路径与命名建议：
  - 目录：`doc/`
  - 文件名：`<database>_doc_part_<序号>.md`（如：`whcenter_doc_part_1.md`、`whcenter_doc_part_2.md`）
- 分片策略（参考值，可由 AI 自动决策）：
  - 单文件最大字符数约 400k（避免 IDE 渲染卡顿）
  - 或单文件最大表数量约 120 张
- 操作要点：
  - 首个分片包含“数据库概要 + 表汇总”
  - 后续分片按表块继续输出（字段、索引、外键）
  - 向用户说明生成了多少分片文件与具体路径
...
- `format`（可选）：`"markdown"` 或 `"pdf"`（PDF 需服务端中文字体 + mermaid-cli）
- `save_path`、`upload_to_oss`（可选）：同 `export_db_doc`
...
- 启用 LLM 时可能含业务域归纳（`llm_er_business_insights`）
- `upload_to_oss: true` 时优先向用户展示 `download_url`，勿在对话中重复贴超大文件
...
**分片输出与本地保存（重要）：**
- 当 ER 图包含大量表/字段，导致单页面渲染困难时，采用“Mermaid 分片保存到本地”的策略。
- 保存路径与命名建议：
  - 目录：`doc/`
  - 文件名：`<database>_er_part_<序号>.md`（如：`whcenter_er_part_1.md`）
- 分片策略（参考值，可由 AI 自动决策）：
  - 单文件最大字符数约 400k
  - 或单文件最大表数量约 120 张
- 操作要点：
  - 每个分片文件仅包含一个 ` ```mermaid ` 代码块，便于单独渲染与导出
  - 外键与推断关系尽量追加在最后；如超限则自动开始新的分片
  - 向用户说明生成了多少分片文件与具体路径
...
- `database`（可选）
- `output_type`（可选，默认 `"both"`）：`"mermaid"` / `"text"` / `"both"`
- `format`、`save_path`、`upload_to_oss`（可选）：同 `export_db_doc`
...
### 3.4 generate_db_rule — 生成数据库 RULE.md

**何时使用：** 用户需要持久化「库表规则 / 报表上下文」、或要求生成 `RULE.md` 供后续智能体记忆时。

**参数：**
- `connection_id`（必需）
- `database`（可选）
- `save_to_project`（可选，默认 true）：写入 `rules/<database>/RULE.md`（相对 `project_base`）
- `save_to_admin`（可选，默认 true）：写入管理库 `db_rules` 表
- `project_base`（可选）：项目根路径，默认当前工作目录
- `title`、`version`、`tags`（可选）
...
**返回解读：**
- 含生成路径、`content` 摘要或保存结果
- 内容涵盖：实体字段、外键/隐含关系、数据流要点（由 `db_rule_tool.py` 聚合元数据生成，**不依赖 LLM**）

**最佳实践：**
- 大型库生成耗时较长，先与用户确认 `database` 范围
- 生成后告知本地路径，便于用户加入 Cursor Rules 或版本管理

---

### 3.5 render_report_oss — 报表导出（OSS）

**何时使用：** 用户要「导出 Excel/CSV 报表」「按条件查库生成文件并给下载链接」时。

**⚠️ 需要服务端配置 `object_storage`（STS/OSS）。未配置会失败。**

**参数：**
- `connection_id`（必需）
- `source`（必需）：`{ "table", "joins"?, "where"?, "order_by"?, "limit"? }`
- `fields`（必需）：`[{ "expr" 或 "column", "label"?, "alias"?, "width"? }]`
- `database`、`file_name`、`output_format`（`xlsx` / `csv`，默认 xlsx）可选
...
```json
{
  ...
  "fields": [
    {"column": "id", "label": "订单号"},
    {"column": "amount", "label": "金额"}
  ]
}
```

**返回解读：**
- 成功时含 OSS `download_url`（或等价字段），**必须把链接交给用户**
- 服务端执行 SELECT 组装报表，注意 `limit` 控制数据量

**最佳实践：**
- 写 `where` 前用 `describe_table` 确认字段名
- 复杂统计可先 `execute_query` 验证 SQL，再填入 `source` / `fields`
- 与 `.trae/skills/warehouse-report/SKILL.md` 等仓库技能配合时使用本工具
...
- `ai_suggestions`（用法 B 且 LLM 启用时）：补充建议
...
- `ai_analysis`（LLM 启用时）：调优报告正文
- `ai_usage`（LLM 启用时）：`model`、`prompt_tokens` 等

**最佳实践：**
- 首次排查用 `full`，然后根据 suggestions 的提示深入某个维度
- 向用户展示时优先摘要 `suggestions` 中 severity=high 的项，再引用 `ai_analysis` 补充说明
...
- `rewrite_suggestions`：SQL 改写建议
  ...
  - 无 LIMIT → 加 LIMIT
  - 大 OFFSET → 游标分页
- `overall_assessment`：综合评价（含是否启用 AI）
- `ai_assessment`（LLM 启用时）：深度审查 Markdown
- `ai_usage`（LLM 启用时）：token 与 `model`

**最佳实践：**
- 先分析，再根据 `rewrite_suggestions` / `ai_assessment` 改写 SQL，可二次调用对比
- 向用户逐条解释 `plan_issues` 与建议；加索引前须用户确认并用 `execute_sql`（需 DDL 权限）
...
- `suggestions`：调优建议
- `ai_analysis` / `ai_usage`（LLM 启用时）：参数调优长文与用量
...
3. 展示 Markdown 文档内容
4. 如果文档过大 → 使用分片策略保存到本地：`doc/<db>_doc_part_X.md`
5. 或用户说"导出 PDF 到 [路径]"：使用 `format="pdf"` 并提示下载/保存
6. AI 告知用户文件已成功写入本地路径
```
...
5. 或采用分片策略保存到本地：`doc/<db>_er_part_X.md`，逐片预览/导出
6. 需要离线 PDF 且已配 OSS：`upload_to_oss: true`，把返回链接给用户
```

### 场景 8：用户说「导出 Excel 报表」
```
1. list_connections → describe_table（确认字段）
2. execute_query（可选，验证筛选与行数）
3. render_report_oss(source + fields, limit) → 返回 download_url
4. 向用户提供 OSS 下载链接，勿在聊天中贴二进制
```

### 场景 9：用户说「生成数据库规则文档」
```
1. list_connections → 确认 database
2. generate_db_rule(save_to_project=true) → 告知 rules/<db>/RULE.md 路径
3. 建议用户将 RULE 纳入项目 Rules 或提交版本库
```

### 场景 10：用户说「这条 SQL 有没有问题」
```
1. analyze_sql → 阅读 explain_plan、plan_issues、rewrite_suggestions
2. 若存在 ai_assessment → 用自然语言总结要点
3. 用户同意优化后 → 给出改写 SQL，必要时 execute_query 验证
```
...
| 「大模型未启用」 | 未配置 LLM | 提示管理员在 Web「大模型配置」激活模型 |
| OSS / 报表失败 | `object_storage` 未配 | 检查 `config.yml` 或改用 markdown 本地输出 |
| 工具超时 | ER/文档库表过多 | 缩小 `database` 范围、`include_columns: false` 或分片 |
